...
- 任务执行产生的临时产物（工具运行目录、缓存、截图、日志等）统一放入 `Agents/{Agent}/tmp/`，不进项目目录、不进 git、不复制到 `deliverables/`；
- 每次完成任务后，Agent 必须清理自身 `Agents/{Agent}/tmp/` 中的临时文件；
- 跨 Agent 的工具 / 进程级临时产物（无归属）放入 `.mapp/tmp/`，由 PM 定期清理；
- 需要留存复核的验证产物写入自身 `Agents/{Agent}/workspace/evidence-{task-id}/`，并在交付说明中引用；
- 项目根目录不出现 `output/`、`dump.rdb` 等无归属产物；运行环境产生的数据（如 Redis 快照）通过配置写入系统临时目录，与项目文件彻底分离。
