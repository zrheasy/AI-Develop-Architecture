...
- 进程级临时产物（工具运行目录、缓存、截图、日志等）统一放入系统 `/tmp` 或 `.mapp/tmp/`，不进项目目录、不进 git、不复制到 `deliverables/`；
- 需要留存复核的验证产物写入自身 `Agents/{Agent}/workspace/evidence-{task-id}/`，并在交付说明中引用；
- 交付前清理工作区中无引用的临时文件；项目根目录不出现 `output/`、`dump.rdb` 等无归属产物；
- 运行环境产生的数据（如 Redis 快照）通过配置写入 `/tmp`，与项目文件彻底分离。
